...
def main():
    ...
    rospy.Subscriber('/mobile_base/commands/velocity', Twist,calculate_pos, queue_size= 1)
...
def ekf():
    global P, Q, R ,last_position,num_tags,last_cmd_info , apriltag_measurement, last_cmd_info, last_odometry,ekf_r,x_state,tag_detected
...
def ekf_predict(x_t, u_t, P_t, Q):
    ...
    F = np.array([[1, 0, - delta_s * sin(x_t[2] + delta_omega)],
        [0, 1, delta_s * cos(x_t[2] + delta_omega)],
        [0, 0, 1]])
...
def calculate_pos(vel_data):
    ...
    global last_cmd_info , prev_call
...
def write_csv():
    ...
    t = rospy.Time.now()
    with open('data_set_big_map.csv', mode='a') as dataset_file:
        data_writer = csv.writer(dataset_file, delimiter=',', quotechar='"')
